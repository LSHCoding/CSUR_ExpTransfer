...
<!-- 移除的 scoping 句（§3.3 disclaimer,注释保留备 §8 与 reviewer 复核;前文"协同演化本身作为被贡献的设计要素"已隐含此意,故从正文删去）：标准 actor–critic 中 critic 的常规联合更新不归入本类,只有 Evaluator 与 Policy 的协同演化本身作为被贡献的设计要素时才计入。 -->
...
<!-- RLAnything [Wan26u]：composite_describe.md 判为跨类。reward–policy 共演化属本节;但 failure-driven task difficulty adjustment 作用于经验集合分布,接近 §7.3;其 environment 改写（LLM task rewriting）是 §3–§6 七条 pathway 之外的新转化形式。此处保留在 §7.1,因 reward–policy 共演化是其定义性特征;跨类成分的系统讨论见 §7.4 与 §8。 -->
...
<!-- Self-Guide [Wan26aj]：composite_describe.md 判为边界。无独立 learned evaluator 参数,严格不满足"Evaluator 参数随 Policy 刷新"的硬判据,故作边界案例标注;其 token→intrinsic-reward 映射亦属七条 pathway 之外的新形式（与 RLAnything 的 environment 改写、RetroAgent 的 trajectory→intrinsic-reward 同属此类新形式,可在 §8 或 §2 统一归并讨论）。 -->
...
<!-- 候选纳入（现稿未含,composite_describe.md 收录,因边界/跨类暂未擅自写入正文,待定夺）：
  · Sar24b [Sar24b]（VLM Agents Generate Their Own Memories）：describe.md 判边界 §7.2。VLM 把 noisy trajectory 抽象为含因果关系、对象状态变化、子目标、视觉关键点的 multimodal programs of thought,经 HITL 验证后既作 RAG memory 检索、也可 SFT 入小模型;但其主产物是可检索 memory artifact（属 §3 territory）,内化进 policy 仅为分支用途,本节匹配度弱。建议：归 §3 或仅在此处以一句 boundary 提及。
  · RetroAgent [Zha26]：describe.md 判跨类,核心 P1→P5↺。language lesson 分支近本节内容改写,numerical intrinsic reward + retrieval 分布控制近 §7.3,含 trajectory→intrinsic-reward 新形式。若纳入,建议放本节末作 boundary,并注释其 §7.3 与新-pathway 成分。 -->
