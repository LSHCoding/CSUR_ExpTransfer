...
在 reasoning 与 general 域，Generative Reward Models [Mah24b]<!-- TODO: pathway 归类存疑,describe 标为不属于 P4,因训练信号来自通用偏好数据而非 agent 决策经验 --> 把 reward modeling 改写为 next-token generation，用 SFT 与 STaR 自举 rationale 训练带推理链的 CoT-GenRM。S2J [Sun25l]<!-- TODO: pathway 归类存疑,describe 标为边界,因监督锚定 pairwise preference 而非 agent trajectory --> 指出 generative reward model 存在 solving 与 judging 能力的落差，要求 Evaluator 判断前先自行求解，用 solving reward 与 judging reward 的联合 RL 目标更新参数。J4R [Xu25i]<!-- TODO: pathway 归类存疑,describe 标为边界,因监督对象是 reasoning answer pair 而非 agent 交互轨迹 --> 用 EIS-GRPO 训练 pairwise judge，通过 response order swap 等等价初始状态上的 judgment correctness 驱动 RL，压低位置偏置与表面格式干扰。Critic-RM [Wan25s] 让模型自生成 critique 作为 latent rationale，以联合目标同时优化 critique generation loss 与 Bradley-Terry preference loss。<!-- TODO: describe 中将该工作标为不属于 P4,理由是训练信号来自通用偏好数据而非 agent 决策经验;citation key 在 describe 中为 [Yu24],需人工核对 -->

面向"让反馈帮助下游修复"的目标，CTRL [Xie25c] 面向 code，以 sandbox 隐藏单测的通过情况为奖励，先用 execution-guided synthetic critique 做 SFT，再用 GRPO 直接优化 critique 使其更能帮助 generator 产出通过测试的代码。RL4F [Aky23]<!-- TODO: pathway 归类存疑,describe 标为不属于 P4,因训练对象是通用 NLP 输出修复而非 agent 交互经验 --> 训练 critique generator，以 refined output 相对 ground truth 的任务指标为 PPO 奖励。AgentV-RL [Wan25aq]<!-- TODO: pathway 归类存疑,describe 标为边界,citation key 在 describe 中为 [Zha26ac] --> 把 verifier 做成多轮 tool-augmented 验证 agent，先做 rejection fine-tuning 学高质量 verifier 轨迹，再用 GRPO 以最终 verdict 与 ground truth 的一致性为奖励。
...
AgentTuning [Zen23d] 将多个环境中的 GPT-4 agent trajectories 统一为 thought-action supervision 用于 generalized agent tuning。Explorer [Pah25] 与 InSTA [Tra25]<!-- TODO: pathway 归类存疑,describe 将该工作归入 P7 而非 P5,需人工确认其是否属于 policy internalization --> 通过 exploration-driven 流水线让强模型在真实网页中生成并验证 success trajectories，再蒸馏给学生模型。AgentTrek [Xu24] 把 web tutorials 转写为可执行的 procedural guidance，通过 guided replay 合成 demonstrations。
