...
Corrective critique 生成供下游 generator 或 actor 取用的修正性意见。CTRL [Xie25c] 以单测通过为奖励用 GRPO 优化 critique，使 generator 更可能产出通过测试的代码。Co-Evolving Critics [Li26l] 让 critic 给出 diagnosis、actor 据此重写轨迹，critic 奖励含 saturation-aware gain 以防随 policy 进化而失效。<!-- RL4F precursor：训练对象是通用 NLP 输出修复而非 agent 交互经验 -->RL4F [Aky23] 以 refined output 相对 ground truth 的任务指标为奖励训练 critique generator，是 critique-driven repair 的早期形态。
