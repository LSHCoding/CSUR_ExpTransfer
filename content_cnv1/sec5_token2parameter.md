# §5 Tokenized-to-Parametric Experience Transformation
...
**代表性工作。** [Pat24] 通过 environment error 与 self-critique 过滤 self-generated web trajectories，再进行 supervised self-improvement。[Pap25b] 在 AndroidWorld 中保留成功 rollout，继续以 SFT loss 进行迭代更新。[Pan24] 利用 unit-test outcome 从 software engineering trajectories 中筛选成功样本，通过 rejection-sampling fine-tuning 完成 policy update。

另一部分工作更强调在参数更新之前对 trajectory 本身进行重构或增强。[Lai25d] 利用 StepCritic 将 Android trajectories 分解为 subgoals，结合 trajectory augmentation 扩展高质量示范。[Hu25i] 将 reflection、branching 与 rollback 中隐含的推理过程重构为显式 Chain-of-Thought，使 reasoning 与 action 能被联合模仿。[Mur24b] 通过 hindsight relabeling 将 exploratory browser interaction 转化为 instruction-conditioned demonstrations。[Xu24] 将 web tutorials 转写为可执行的 procedural guidance，通过 guided replay 合成 demonstrations。[Pah25] 与 [Gan25b] 先通过结构化网页探索生成并验证 success trajectories，再将其蒸馏为学生模型的监督数据。[Zen23d] 将多个环境中的 GPT-4 agent trajectories 统一为 thought-action supervision 用于 generalized agent tuning。
