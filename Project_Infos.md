...
- 文本形式：raw trajectories、reflections、summaries、rules、insights、hints、guidelines、skill descriptions
...
| **Abstraction level** | raw / refined                                                          | 区分 raw trajectories 与 refined derivatives；主要在 Tokenized 层内体现 |
| **Experience Source**            | agent self-generated / human demonstration / teacher model synthesized | 用于讨论经验来源对 pathway 选择的影响                                      |


#### 2.2.6 边界澄清

**Raw experience 的定位**：Raw trajectories 本身是最低抽象程度的 Narrative Tokenized 载体（"零度抽象"特例），而非独立于 carrier 体系之外的东西。Transformation 通常以 raw 为起点，目标是提升抽象程度（Narrative refined）、改变形式化程度（Narrative → Schematic）或改变存在层次（Tokenized → Latent / Parametric）。
...
```
   ...
   (reflections,          (code,        (KV cache,            (VLA, LLM         (RM, PRM,
    raw logs,              workflows,    soft prompts,          agents)            verifier)
...
过程 $\mathcal{T}: \mathcal{C}*\text{src} \rightarrow \mathcal{C}*\text{tgt}$ 构成 Experience Transformation，当且仅当同时满足：

1. **经验语义锚定（Experience Grounding）**：源内容可映射为 $e=(c,a,o,f)$ 或由其派生。
2. **经验内容承载（Experience Embodiment）**：目标载体编码了源经验的语义内容。
...
2. **严格把控边界**：纳入判定检验异构动作空间条件；转化判定检验经验语义锚定（条件 1）与经验内容承载（条件 2）。
